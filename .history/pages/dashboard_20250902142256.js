...
function Dashboard() {
  ...
  const [isopendelete,setIsModalDelete] = useState(false)
  ...
  return (
    <>
              ...
              <div className={styles.flex1}>
                <td>
                  <button className={styles.btnedit}>
                    ...
                    />
                    <button
                      onClick={() => {setSelectProduct(p),setIsModalDelete(true)}}
      ...
      {isopendelete && (
        <DeleteProductModal productId={selectProduct?.id}
        onclose={()=>setIsModalDelete(false)}
        ondelete = {(id)=> deleteProduct} />
      )}
    </>
  );
}
